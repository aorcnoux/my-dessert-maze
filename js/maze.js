...
class Maze {
    ...
    constructor(options) {
        ...
        this.ballDia = options.ballDia || this.step;
        ...
        this.mazeGrids = [];
        this.wallPattern = null; // 🆕 儲存牆的圖案

        this.entrance = { x: 1, y: 0 };
        this.exit = { x: this.w - 2, y: this.h - 1 };
    ...
    initMaze() {
        ...
        // 🟤 先 preload 圖片 → 圖片 load 完才畫
        const img = new Image();
        img.src = "./img/choco_wall.png";
        img.onload = () => {
            this.wallPattern = this.cvsCtx.createPattern(img, 'repeat');
    
            // 圖片 ready 之後再開始畫
            this.drawWall("choco-wall", "white");
    
            elMaze.style.background = "#4D4040";
    
            this.drawPath(entrance, { x: 1, y: -1 }, "#e0f2f1");
    
            this.mazeGrids.forEach((row) => {
                row.forEach((cell) => {
                    if (!cell.isPath && !cell.isEntrance && !cell.isExit) {
                        this.fillGrid(cell.x, cell.y, "choco-wall");
                    }
                });
            });
    ...
    fillGrid(x, y, color) {
        ...
        if (color === "choco-wall") {
            if (!this.wallPattern) {
                const img = new Image();
                img.src = "./img/choco_wall.png"; // 你的巧克力牆圖
                img.onload = () => {
                    this.wallPattern = ctx.createPattern(img, 'repeat');
    ...
    drawBall(elBall, d) {
        ...
        elBall.style.transform = "scale(3)";
        elBall.style.transformOrigin = "center center";
    }
...
function reGenMaze() {
    ...
    if (maze && typeof maze.stopMove === "function") {
        maze.stopMove();  // 先清掉上一輪事件 + isGameStarted
    }
    // 重新生成迷宫并移动小球
    maze = genMaze({
        ...
        // 等 100ms 確保 drawBall 已完成，再 allow start
        setTimeout(() => {
            maze.isGameStarted = false; // 確保還沒開始
        }, 100);
}
